...
function Grid(grid, handleGridSize, inputCells, setCells, inputRows, setRows) {
        ...
        <main>
            ...
            {rows.map((row, index) => (
                <ul className="row" key={index}>
                    {cells.map((cell, index) => <li key={index} className="item" onClick={handleClick} />)}
                </ul>
            ))}
